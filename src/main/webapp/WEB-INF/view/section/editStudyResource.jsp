    ...
    <div class="row" >
        <div class="col-md-12 main" style="margin-left: 15px;">
            <div class="mainContent">
                <form class="form-horizontal" id="studyResourceForm" >
                    <div class="form-group">
                        <input type="hidden" class="form-control" id="id" name="id" value="${studyResource.id}" >
                        <label for="title" class="col-sm-2 control-label">标题</label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="title" name="title" placeholder="请输入标题" value="${studyResource.title}">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-3">
                            <select name="category.id" class="form-control">
                                <c:forEach var="category" items="${categoryList}">
                                    <option value="${category.id}" <c:if test="${category.id eq studyResource.category.id}"> selected="selected"</c:if>>${category.categoryName}</option>
                                </c:forEach>
                            </select>
                    ...
                    <div class="form-group">
                        <div class="col-sm-offset-8 col-sm-2" style="overflow: hidden">
                            <button type="button"  class="btn btn-success" style="width: 100px;display: block;float: right" onclick="submitEdit()">提交</button>
                        </div>
                    </div>
                </form>
            </div>
...
<script type="text/javascript">
    ...
    ue.ready(function() {
        //设置编辑器的内容
        ue.setContent('${studyResource.content}');

    });
    function submitEdit(){
        $.ajax({
            type:'post',
            url:'${prefix}/rest/studyResource?_method=put',
            data:$('#studyResourceForm').serialize(),
            success:function(data) {
                if(data.status!=-1){
                    window.location.href='${prefix}/rest/studyResource/'+data.status;
                }else{
                    alert(data.msg);
                }
            }
